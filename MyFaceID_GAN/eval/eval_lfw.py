...
def evaluation_1_fold(feature_path='./result/cur_epoch_result.mat'):
    ACCs = np.zeros(10)
    result = scipy.io.loadmat(feature_path)
    for i in range(10):
        fold = result['fold'] #注意fold是一个list
        flags = result['flag']
        featureLs = result['fl']
        featureRs = result['fr']

        valFold = fold != i#用来得到阈值的那部分数据集
        testFold = fold == i#用来做实际测试的那部分数据集
        flags = np.squeeze(flags)
...
def getFeatureFromTorch(feature_save_dir, net, device, data_set, data_loader):
    ...
    for data in data_loader:
        ...
        #print('LFW data size: ', data[0].size())
        #结果是(3, 128, 128)
        with torch.no_grad():
            res = [net(d)[1].data.cpu().numpy() for d in data]
